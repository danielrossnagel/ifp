...
logging.info('Eager execution is:')
answr = tf.executing_eagerly()
logging.info(answr)
...
def create_dataset():
    ...
    particle_type_tensor = tf.convert_to_tensor(particle_type, 
                                                dtype=tf.int64, 
                                                name='particle_type')
    key_tensor = tf.convert_to_tensor(key, 
                                      dtype=tf.int64, 
                                      name='key')
    position_tensor = tf.convert_to_tensor(position, 
                                           dtype=tf.float32, 
                                           name='position')
    n_particles_per_example_tensor = tf.convert_to_tensor(n_particles_per_example, 
                                                          dtype=tf.int32, 
                                                          name='n_particles_per_example')
    is_trajectory_tensor = tf.convert_to_tensor([True,], 
                                                dtype=tf.bool, 
                                                name='is_trajectory')
    target_position_tensor = tf.convert_to_tensor(target_position, 
                                                  dtype=tf.float32, 
    ...
    dataset = tf.data.Dataset.from_tensors((feat))
